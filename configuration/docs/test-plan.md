...
| **HW-2** | `T_detect` (§12.2) | **`MODE_SWEEP`.** Step `armOffsetUs` late in 500 µs increments until reception fails; the failure edge gives `T_detect` directly, since a window armed `l` late catches the frame iff `l ≤ G`. Sweep early too — the early edge returns `G` independently, and the two must sum to `N·T_sym − T_detect`. **No scope needed**, and this is the B3 blocker. | bench node |
