...
| **B0** | ~~GPIO light-sleep wakeup on DIO0 **and** DIO1, disarmed in step with `gpio_intr_disable` (§2.5)~~ — **WRITTEN, NOT VERIFIED.** `LoraInterface::wakeSourceEnable/Disable`, paired one-for-one with the four existing `gpio_intr_enable`/`disable` sites; the ISR disarms nothing (`gpio_wakeup_disable` takes a spinlock and is not IRAM-resident), so the handler task does it on dequeue. **No ESP-IDF toolchain was available: this is not compiled and not run.** | timestamps lose their 100 ms-scale outliers, jitter < 1 ms, **and light-sleep residency is unchanged** — the residency half is the one this change could plausibly break, and it needs hardware |
