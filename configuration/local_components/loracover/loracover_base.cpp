...
LoraCovPacket *LoraCovEncoder::get_battery_level_request() {
  uint8_t data = 0x1;
  return this->encode_(0xA2, &data, 1);
}

LoraCovPacket *LoraCovEncoder::get_light_level_request() {
  uint8_t data = 0x1;
  return this->encode_(0xAA, &data, 1);
}

LoraCovPacket *LoraCovEncoder::get_position_request() {
  uint8_t data = 0x1;
  return this->encode_(CMD_GET_POSITION, &data, 1);
}

LoraCovPacket *LoraCovEncoder::get_send_pin_request(uint16_t pin) {
  uint8_t data[2];
  data[0] = (pin & 0xFF00) >> 8;
  data[1] = pin & 0xFF;
...
LoraCovPacket *LoraCovEncoder::get_open_request() {
  uint8_t data = 0xDD;
  return this->encode_(CMD_SET_STATE, &data, 1);
}

LoraCovPacket *LoraCovEncoder::get_close_request() {
  uint8_t data = 0xEE;
  return this->encode_(CMD_SET_STATE, &data, 1);
}

LoraCovPacket *LoraCovEncoder::get_stop_request() {
  uint8_t data = 0xCC;
  return this->encode_(CMD_SET_STATE, &data, 1);
}

LoraCovPacket *LoraCovEncoder::get_set_position_request(uint8_t position) {
  return this->encode_(CMD_SET_POSITION, &position, 1);
}

void LoraCovEncoder::checksum_() {
  uint8_t checksum = 0;
  int i = 0;
...
LoraCovPacket *LoraCovEncoder::encode_(uint8_t command, uint8_t *data, uint8_t length) {
  ...
  this->checksum_();
  ESP_LOGV("LoraCov", "ENC(%d): 0x%s", packet_.length, pkt_to_hex(packet_.data, packet_.length).c_str());
  return &this->packet_;
}
...
void LoraCovDecoder::decode(const uint8_t *data, uint16_t length) {
  ...
  ESP_LOGV("LoraCov", "DEC(%d): 0x%s", length, pkt_to_hex(data, length).c_str());
